...
dtls_handle_read(dtls_context_t *ctx) {
	...
	if (uip_newdata()) {
		...
		PRINTF("Client received message from "); PRINT6ADDR(&session.addr); PRINTF(":%d\n", uip_ntohs(session.port));
...
init_dtls(session_t *dst) {
	...
	PRINTF("set connection address to "); PRINT6ADDR(&dst->addr); PRINTF(":%d\n", uip_ntohs(dst->port));
